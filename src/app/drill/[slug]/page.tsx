...
function getStimulusDurationMs(q: Question, topicSlug: string): number {
  if (topicSlug === "tachistoscope")
    return getTachistoscopeFlashMs(q.difficulty);
  if (topicSlug === "working-memory")
    return getWorkingMemoryShowMs(q.difficulty);
  return 0;
}
...
export default function DrillPage({
  ...
  function handleAnswer(answer: string) {
    ...
    const elapsed = Date.now() - startTimeRef.current;
    const { isCorrect, earnedXp, newCombo } = useDrillStore
      .getState()
      .answerQuestion(answer, elapsed);
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-background gap-4">
        {isTachy ? (
          ...
          <div className="space-y-6 text-center">
            ...
            <StimulusCountdownBar
              durationMs={getStimulusDurationMs(store.currentQuestion!, slug)}
            />
          </div>
        )}
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="glass rounded-xl p-3">
              <div className="text-xl font-bold text-primary">{r.xpEarned}</div>
  ...
  return (
    <div
      className={cn(
        "min-h-screen flex flex-col transition-all duration-500 bg-gradient-to-b",
        comboGrad(store.combo),
        ...
        <div className="h-2 rounded-full bg-secondary overflow-hidden">
          <div
            className={cn(
              ...
              danger ? "progress-bar-danger" : "progress-bar",
            )}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      {store.personalBest && store.personalBest.bestScore > 0 && !isAns && (
        <div className="px-4 mt-2">
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
            ...
            <span>
              Rekorun: {store.personalBest.bestScore} XP | Seri:{" "}
              {store.personalBest.bestStreak}
            </span>
          </div>
        </div>
      )}
